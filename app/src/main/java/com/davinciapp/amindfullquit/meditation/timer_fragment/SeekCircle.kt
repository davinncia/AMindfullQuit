...
/**
 *
 * View representing the timer setting.
 * Allow to choose a value between 1 & 30 then show progress animation.
 */
...
class SeekCircle : View {
    ...
    //VALUES
    private val maxProgress = 20 //Minutes
    ...
    private fun updateProgress(angle: Int) {
        ...
        val progress =
            if (angle in -180..-89)
                maxProgress * 3 / 4 + ((angle + 180) / progressRatio) + 1  //Top left part of circle is inverted
            else
                (angle + 90) / progressRatio + 1
